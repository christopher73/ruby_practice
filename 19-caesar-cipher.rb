...
def caesar_cipher(offset, string)
  ...
  (0...news.length).each do |i|
    
    strr=news[i].split(//)

        (0...strr.length).each do |j|

          if 122%strr[j].ord >=offset
          strr[j] = (strr[j].ord + offset).chr
          else
            strr[j] = (96+(offset - 122%strr[j].ord)).chr
          end
        end
      news[i]=strr.join
      end
